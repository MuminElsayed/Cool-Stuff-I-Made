...
//A player controller that can switch between top-down and FPS aim/moving modes using the new input system.
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        int playerWeapon = PlayerPrefs.GetInt("PlayerWeapon", 0); //Get current weapon from weapons class
    ...
    void Update()
    {
        ...
        if (!topDownMode) //FPS move/aim
        {
            ...
            lookVert += look.y;
            lookHor = look.x;
            lookVert = Mathf.Clamp(lookVert, -89f, 89f); //Locked to top/bottom angles
